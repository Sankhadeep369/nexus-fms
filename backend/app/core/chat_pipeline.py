...
class ChatPipeline:
    ...
    def _agent_incident_triage(
        ...
        if triage.succeeded and triage.escalation_email:
            ...
            # Lead with occupant safety actions — generated by the LLM from the
            # actual incident so they're PROPORTIONATE (a lighting fault is not an
            # electrocution). Best-effort: falls back to a generic checklist only if
            # the LLM is unavailable; the hazard here is inferred from the incident
            # wording (never the domain, which would over-escalate).
            from app.core.agents.immediate_actions import build_immediate_actions, detect_hazard

            incident_text = triage.summary or retrieval_query
            fallback_hz = detect_hazard(incident_text) or "general"
            actions = build_immediate_actions(
                "", fallback_hz, settings.groq_api_key, settings.groq_model,
                allow_llm=True, context=incident_text,
            )
            final_answer = (
                f"## Incident Summary\n\n"
        ...
        # Triage couldn't complete (e.g. the triage service was briefly unavailable).
        # Return a fast, safe message instead of falling into the multi-minute CPU SLM.
        msg = (
            "I couldn't complete the incident triage just now — the triage service may be "
            "busy. Please try again in a moment.\n\n"
            "If this is an emergency, follow your site's emergency procedures and call "
            "emergency services if there is any risk to life."
        )
        yield {"type": "token", "text": msg}
        yield {
            "type": "done",
